...
```text
场景清单 -> 测试矩阵 -> 人工/AI 探索 -> 缺陷台账 -> 分级 -> 修复 -> 回归测试 -> 发布前冒烟测试
```
...
- 空列表/空表格异常
- 文件不存在、格式错误、权限不足
每发现一个问题，按缺陷台账模板记录；如果本仓库还没有台账，首次使用前先创建 `BUGS.md` 或本次测试指定的缺陷记录文件。不要直接修复。
```
...
首次使用时创建根目录 `BUGS.md` 或本次测试指定的缺陷记录文件，并持续维护为缺陷台账。所有发现项进入该文件。不要把“猜测的根因”当作事实，必须区分：
...
```text
请读取本轮已创建的缺陷台账（例如 BUGS.md 或本次测试指定的缺陷记录文件），按严重程度排序。先不要改代码，只分析每个 bug 的可能根因、影响范围、推荐修复策略和需要补充的回归测试。
```

如果 `BUGS.md` 尚不存在，先创建台账并写入本轮已复现问题；如果没有已复现问题，则不要为了占位制造空台账。
