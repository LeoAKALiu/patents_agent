...
普通用户从 `工作台` 进入三条起步路径：从技术想法撰写发明专利、从结构方案撰写实用新型、导入已有稿件进行润色提升。正文修复、版本链路、知识库和导出门禁被拆成独立工作区；语料库、护城河、会审、质检等复杂能力仍通过 `专家工具` 归集。

实现落点：

- `frontend/src/guidedFlow.ts` 中的 `v1StartChoices` 定义三条默认入口。
- `frontend/src/guidedFlow.ts` 中的 `mainSections` 暴露 `工作台 / 项目 / 文稿与修复 / 知识库 / 专家工具 / 导出 / 设置`。
- `frontend/src/app/routes.tsx` 将旧 `generate`、`utility` 和专家导出状态迁移到新的工作区路由。
- `frontend/src/features/documentRepair/` 和 `frontend/src/features/export/` 分别承载文稿修复与正式导出。
...
| 菜单 | 入口 ID | 作用 |
| --- | --- | --- |
| 工作台 | `workbench` | 当前项目命令中心，显示下一步、流程进度、风险和三条起步路径 |
| 项目 | `projects` | 查看历史项目、筛选、选择和删除项目 |
| 文稿与修复 | `documents` | 管理内部初稿、问题队列、标注式修复、版本链路和导出门禁总览 |
| 知识库 | `knowledge` | 进入语料库建设、知识库检索和材料复用工作区 |
| 专家工具 | `expert` | 打开护城河、会审、质检、完善、审查修改等专家工具 |
| 导出 | `export` | 分离导出正式提交稿、内部复核材料和风险说明 |
...
| 导出状态 | 常驻 | 显示正式导出是否已放行或被阻断 |
| 诊断面板 | 常驻 | 查看后端、模型和智能体运行状态 |
...
5. 在 `文稿与修复` 中处理高风险项、补强建议和标注式修复。
6. 生成正式稿，运行成稿会审。
7. 在 `导出` 工作区确认正式稿、内部复核材料和风险说明。
8. 只有导出门禁全绿时下载正式提交稿。
9. 将正式稿交由专利代理师、律师或专业人员复核。
...
| 工作台、三选一和项目工作区 | `frontend/src/features/workbench/WorkbenchWorkspace.tsx`、`frontend/src/views/projectViews.tsx`、`frontend/src/features/projects/ProjectWorkspace.tsx` |
| 向导步骤面板 | `frontend/src/flow/panels/` |
| 文稿修复和导出工作区 | `frontend/src/features/documentRepair/DocumentRepairWorkspace.tsx`、`frontend/src/features/export/ExportWorkspace.tsx` |
| 知识库和专家工具工作区 | `frontend/src/features/knowledge/KnowledgeWorkspace.tsx`、`frontend/src/features/corpus/CorpusWorkspace.tsx`、`frontend/src/features/quality/QualityWorkspace.tsx`、`frontend/src/features/postDraft/PostDraftWorkspace.tsx` |
