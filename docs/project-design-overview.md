...
| Short SHA | `3868f0dc` |
| Worktree 状态 | 写入本文档前干净；写入后新增 `docs/project-design-overview.md` |
...
系统允许用户登记可行但未验证的技术路线，用于布局和分案策略，但不会把未验证方案伪装成已验证事实。候选发明点、证据绑定和完善矩阵都带有证据状态。
...
正式提交稿只允许包含清理后的摘要、权利要求书、说明书和附图说明。内部策略、护城河、会审日志、支撑缺口、补强建议和风险说明必须留在内部稿或侧车报告中。
...
- `DisclosureRun`：交底生成运行记录，包含候选发明点、支撑材料摘要和运行状态。
