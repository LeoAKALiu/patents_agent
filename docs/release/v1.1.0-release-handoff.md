...
- desktop smoke reports backend health ok, diagnostics preload ok, and production renderer assets loaded
...
The current supported release artifact is still a verified compiled desktop bundle, not an automatically signed/notarized DMG.
...
2. Build desktop main/preload assets and run smoke.
...
3. Save release proof artifacts.
   ...
   - terminal log showing `desktop run smoke` with production renderer assets loaded
...
4. Package only after smoke proof is captured.
   ...
   - built Electron main/preload from `desktop/dist-electron/`
...
- Backend runtime is still the local Python environment supervised by Electron.
