...
3. **打包**：优先运行 `scripts/package_dmg.sh`。脚本会构建 Tauri DMG、生成带 SHA/时间戳的交付副本，并写出 `.artifacts/dmg/<timestamp>-<sha>/report.md`。
4. **验证**：默认脚本已执行 `hdiutil verify`、DMG smoke 和 Tauri DOM smoke；合并或正式交付前使用 `scripts/package_dmg.sh --full` 额外跑完整 `scripts/v1_smoke.sh`。
