...
describe("agent provider card helpers", () => {
  it("keeps required providers selected and removes unavailable optional providers", () => {
    const selected = normalizeAgentSelection(doctor, ["kimicode", "gemini"], "formula");
    expect(selected).toEqual(["codex", "deepseek", "claude", "kimicode"]);
  });
  ...
  it("allows toggling optional providers with auth_status=unknown when installed", () => {
    ...
    const selected = normalizeAgentSelection(doctorWithUnknown, ["kimicode", "gemini"], "formula");
