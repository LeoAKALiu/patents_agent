...
├── src-tauri/            Tauri v2 macOS 桌面壳与本地后端 sidecar 管理
├── tests/                后端单元与接口测试
...
- Desktop：Tauri v2 + React/Vite，本地启动 FastAPI sidecar。
...
v1.1.0 release candidate 的桌面产物是 Tauri macOS bundle/DMG。当前默认构建使用 ad-hoc 签名，尚未完成 Developer ID 签名、公证或自动更新。

- `frontend/dist/`：React 渲染端静态 bundle。
- `src-tauri/target/release/bundle/dmg/`：Tauri 生成的本地 DMG。

release manager 可基于 Tauri 产物完成签名、公证和分发。收到已打包的 macOS 应用后：
...
`src-tauri/` 是唯一桌面运行时。Tauri 进程会启动本地 Python/FastAPI 后端，等待 `http://127.0.0.1:<port>/api/health` 通过后再加载 React 前端；生产模式加载 `frontend/dist/index.html`，并通过 Tauri command 将渲染端的 `/api/*` 请求指向本地后端：
...
v1.1.0 release branch 额外加入桌面启动诊断和 DOM smoke：启动失败、前端资源缺失、渲染进程崩溃和后端 health 结果会记录到 Tauri startup log。生产模式若 React 根节点、sidebar 或 topbar 未渲染，DOM smoke 会以失败退出。
...
v1.1.0 发布烟测（推荐 release gate 单命令）：

```bash
bash scripts/v1_smoke.sh
```

该脚本会依次运行后端 `python3 -m pytest -q`、基于 golden cases 的确定性 API smoke、前端 `npm test -- --run`、前端 build，以及 Tauri `cargo check` / `cargo test`。整个 API smoke 使用本地 fake LLM，不读取 `.env`，不调用真实 LLM API，也不需要凭证。
...
- `npm run build`：通过
- `scripts/v1_smoke.sh`：全部通过（后端测试、API 烟测、前端测试、前端构建、Tauri cargo check/test）
...
- **签名、公证和自动更新未完成**：当前仓库可生成本地 Tauri DMG，但 Developer ID 签名、公证和自动更新仍需单独发布流程。
