...
def test_tauri_build_command_runs_frontend_and_backend_packaging_steps() -> None:
    ...
    sidecar = read(ROOT / "scripts" / "build_backend_sidecar.sh")
    ...
    assert '"$PYTHON" -m PyInstaller scripts/backend.spec' in sidecar
    assert "--distpath build/backend" in sidecar
    assert "--workpath build/pyinstaller-work" in sidecar
